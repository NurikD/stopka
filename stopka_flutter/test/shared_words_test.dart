...
void main() {
  testWidgets(
    'shared text opens straight on the parsed draft, and saving works without a key',
    (tester) async {
      final cards = _Cards();
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            aiAvailabilityProvider.overrideWithValue(
              FixedAiAvailability(false),
            ),
  ...
  testWidgets('shared text with no words in it falls back to the paste box', (
    tester,
  ) async {
    await tester.pumpWidget(
      ProviderScope(
