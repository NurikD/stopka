...
void main() {
  const key = PackKey(level: 'B1', grammarTopic: 'Present perfect', interest: 'games');

  group('GeneratedContentSource', () {
    test('returns the validated part and sends that part\'s own prompt', () async {
      final client = _ScriptedClient([asRaw(grammarJson())]);
      final part = await GeneratedContentSource(client, loadPrompt: _prompt).fetchPart(key, PackPart.grammar);
    ...
    test('one invalid answer is retried, a valid second one is used', () async {
      ...
      final part = await GeneratedContentSource(client, loadPrompt: _prompt).fetchPart(key, PackPart.writing);
    ...
    test('two invalid answers end in a plain Russian error, never in stored content', () async {
      ...
      await expectLater(
        GeneratedContentSource(client, loadPrompt: _prompt).fetchPart(key, PackPart.reading),
        throwsA(isA<LlmException>().having((e) => e.messageRu, 'message', contains('чтение'))),
      );
      expect(client.calls, 2);
    });
    ...
    test('a transport error is not retried', () async {
      ...
      await expectLater(
        GeneratedContentSource(client, loadPrompt: _prompt).fetchPart(key, PackPart.writing),
  ...
  group('PackService', () {
    ...
    test('generateMissing fills every part, and a second run costs nothing', () async {
      final source = _MapSource(allGood());
      final service = PackService(repo: repo, source: source, hasKey: () async => true);
      ...
      await service.generateMissing(pack.id, key);
      expect(source.calls.values.every((n) => n == 1), isTrue, reason: 'cached parts are not requested again');
    });

    test('the same key finds the same pack, so reopening a unit spends no requests', () async {
      final service = PackService(repo: repo, source: _MapSource(allGood()), hasKey: () async => true);
      final first = await service.open(key);
      final second = await service.open(key);
      expect(second.id, first.id);
    });

    test('without a key nothing is requested and nothing is marked failed', () async {
      final source = _MapSource(allGood());
      final service = PackService(repo: repo, source: source, hasKey: () async => false);
      final pack = await service.open(key);

      await service.generateMissing(pack.id, key);
      expect(source.calls, isEmpty);
      expect((await repo.getPack(pack.id))!.statusOf(PackPart.reading), PartStatus.pending);

      expect(await service.generatePart(pack.id, key, PackPart.reading), isFalse);
      expect(service.lastError[PackService.slot(pack.id, PackPart.reading)], contains('ключ'));
    });

    test('a failing part is marked failed with its message, the others still get made', () async {
      final scripts = allGood()..[PackPart.reading] = [const LlmException('Лимит запросов.')];
      final service = PackService(repo: repo, source: _MapSource(scripts), hasKey: () async => true);
      final pack = await service.open(key);
      ...
      expect(service.lastError[PackService.slot(pack.id, PackPart.reading)], 'Лимит запросов.');

      // A failed part is retried only when asked.
      scripts[PackPart.reading] = [validatePart(PackPart.reading, readingJson(), level: 'B1')];
      expect(await service.generatePart(pack.id, key, PackPart.reading), isTrue);
      expect((await repo.getPack(pack.id))!.statusOf(PackPart.reading), PartStatus.ready);
    });

    test('"Материал плохой" throws the old part away and generates a new one', () async {
      final first = validatePart(PackPart.writing, writingJson(), level: 'B1');
      final second = {...first, 'task': 'Write 4-6 sentences about your favourite level. Use the present perfect.'};
      ...
      final pack = await service.open(key);
      await service.generateMissing(pack.id, key);
      expect((await repo.getPack(pack.id))!.payloadOf(PackPart.writing)!['task'], first['task']);

      expect(await service.flagAndRegenerate(pack.id, key, PackPart.writing), isTrue);
      expect((await repo.getPack(pack.id))!.payloadOf(PackPart.writing)!['task'], second['task']);
      expect(source.calls[PackPart.writing], 2);
    });

    test('a flagged part whose regeneration fails ends up without content', () async {
      final source = _MapSource({
        ...allGood(),
        PackPart.writing: [validatePart(PackPart.writing, writingJson(), level: 'B1'), const LlmException('Сбой.')],
      });
      final service = PackService(repo: repo, source: source, hasKey: () async => true);
      final pack = await service.open(key);
      await service.generateMissing(pack.id, key);

      expect(await service.flagAndRegenerate(pack.id, key, PackPart.writing), isFalse);
    ...
    test('progress, attempts and mistakes are stored and survive regeneration', () async {
      final service = PackService(repo: repo, source: _MapSource(allGood()), hasKey: () async => true);
      final pack = await service.open(key);
      await service.generateMissing(pack.id, key);

      await repo.logAttempt(pack.id, PackPart.grammar, itemIndex: 0, userAnswer: 'has', isCorrect: true);
      await repo.saveProgress(pack.id, PackPart.grammar, score: 5, total: 6);
      await repo.addMistakes(pack.id, const [
        MistakeInput(skill: 'writing', category: 'tense', original: 'I play', corrected: 'I played'),
      ]);
